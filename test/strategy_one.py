...
#                         test_df["difficulty"] == 90)]
#
#         return df_recall_1_0, df_recall_1_90, df_recall_2_0, df_recall_2_90, df_group_0, df_group_90
#
...
def send_test_again(receive_json):
    ...
    # redis记录答题结果即可、是否正确。
    dict_test = {"user_id": user_id, "practice_id": practice_id, "knowledge_ids_num": knowledge_ids_num,"knowledge_ids_list": knowledge_ids_list,
                 "location_ids_num": location_ids_num, "is_true_list": is_true_list, "false_knowledge_ids_list": false_knowledge_ids_list}
    red.hmset(str(practice_id), dict_test)
    ...
    if knowledge_ids_num == 1:
        ...
        difficulty = judge_next_difficulty(is_true_list)
        if difficulty == 0:
            next_test = df_0[0].sample(1, replace=True)  # 要是没有的话，重复出题。
        elif difficulty == 90:
            next_test = df_90[0].sample(1, replace=True)  # 要是没有的话，重复出题。
        df_next_test = next_test[["test_id", "ids_split"]]

        df_next_test = df_next_test.reset_index()
        df_next_test.index += (location_ids_num + 1)  # 重置索引位置
        next_json = df_next_test.to_json(orient='index', force_ascii=False)
